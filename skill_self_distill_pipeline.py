...
- 命令示例代码块中只写可直接下发的命令：
  1. 剥除源文档中的设备提示符（如 `[~HUAWEI]`、`[*HUAWEI-aaa-authen-auth]`、`<HUAWEI>`），提示符是回显不是输入；提示符携带的视图层级、`*`未提交状态等信息，改用正文文字说明（如"进入authentication-scheme视图后配置，完成后commit提交"）。
  2. 不要在命令行尾追加 `#` 行内注释（设备CLI不识别，整行下发会报错），备选参数或补充说明（如"或radius"）写在代码块外的正文中。
